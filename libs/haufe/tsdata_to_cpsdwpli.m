...
% Toolbox(TM).
%
% Multi-taper routine adapted from <https://urldefense.proofpoint.com/v2/url?u=http-3A__www.chronux.org_&d=DwIGAg&c=-35OiAkTchMrZOngvJPOeA&r=q2b2jk6iMqHvTzFUUrmWpFcLBpvSfdmimXkepsdyNwg&m=WRfVthoq4yzbH_uWxPYgHAEAZbX9xE8d_u_cooFTF-WJupMFJew1m_YJzIqYxP1W&s=sW08cS0D-ltZQrIJ7orVqInY9L3yKW57dAmAaUjLiNg&e=  |Chronux|> code [2]
% authored by <https://urldefense.proofpoint.com/v2/url?u=http-3A__mitralab.org_&d=DwIGAg&c=-35OiAkTchMrZOngvJPOeA&r=q2b2jk6iMqHvTzFUUrmWpFcLBpvSfdmimXkepsdyNwg&m=WRfVthoq4yzbH_uWxPYgHAEAZbX9xE8d_u_cooFTF-WJupMFJew1m_YJzIqYxP1W&s=L5iWKDK5TLn8FRQLkazzUKeJ2ZszbxHMeP97ZyoH0VQ&e=  Partha Mitra> and Kenneth D. Harris.
%
%% References
%
% [1] L. Barnett and A. K. Seth,
% <https://urldefense.proofpoint.com/v2/url?u=http-3A__www.sciencedirect.com_science_article_pii_S0165027013003701&d=DwIGAg&c=-35OiAkTchMrZOngvJPOeA&r=q2b2jk6iMqHvTzFUUrmWpFcLBpvSfdmimXkepsdyNwg&m=WRfVthoq4yzbH_uWxPYgHAEAZbX9xE8d_u_cooFTF-WJupMFJew1m_YJzIqYxP1W&s=4jaHUhQiopOUC27G_RxC6UfqNG2Gmf51TNBhXgOICoA&e=  The MVGC
...
function [S, wPLI] = tsdata_to_cpsd(X,fres,method,window,noverlap,nw,ntapers)
...
if strcmpi(method,'MT')
    ...
    S = permute(S,[2 3 1])/N;
    numer = permute(numer,[2 3 1])/N;
    denom = permute(denom,[2 3 1])/N;
    wPLI = numer ./ denom;
...
elseif strcmpi(method,'WELCH')
    ...
    S = 0; numer = 0; denom = 0;
    for r = 1:N % works for single-trial too
        XX = cpsd_welch(X(:,:,r),n,fres+1,window,noverlap);
        S = S + XX;
        numer = numer + imag(XX);
        denom = denom + abs(imag(XX));
